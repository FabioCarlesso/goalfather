...
class LeagueDslTest {
    ...
    @Test
    fun `RoundBuilder resolve nomes e numera partidas`() {
        val round = RoundBuilder(1).apply {
            match(homeId = 1, awayId = 6)
            match(homeId = 2, awayId = 5)
        }.build(season = 2026, clubNameById = mapOf(1L to "Casa", 6L to "Fora", 2L to "A", 5L to "B"))

        assertEquals(1, round.number)
        assertEquals(2026, round.season)
        assertEquals(RoundStatus.Scheduled, round.status)
        // matchId = número da rodada × 1000 + índice (mesma fórmula do gerador).
        assertEquals(1001L, round.matches[0].matchId)
        assertEquals(1002L, round.matches[1].matchId)
    ...
    fun `league monta o agregado completo e a tabela inicial`() {
        ...
        val standings = league.initialStandings()
